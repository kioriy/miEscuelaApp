...
class TeacherAttendanceController extends Controller
{
    ...
    /**
     * Finalize attendance list (marks remaining non-handled as absent)
     */
    public function finalizeAttendance(Request $request, $classroomId)
    {
        $user = $request->user();
        if ($user->role !== 'teacher') {
            return response()->json(['success' => false, 'message' => 'Unauthorized'], 403);
        ...
        foreach ($students as $student) {
            ...
            if ($logs->has($student->id)) {
                $studentLogs = $logs->get($student->id);
                foreach ($studentLogs as $log) {
                    if (in_array($log->status, ['present', 'late', 'excused', 'absent'])) {
                        $hasHandledRecord = true;
                        break;
                    }
                }
        ...
        return response()->json([
            'success' => true,
            'message' => "Attendance finalized. $markedAbsent students marked as absent."
        ]);
    }
}
